...
- 판단 기준 예시:
  * 거래량 급증 + 15분봉 음봉 마감 → 고점 분배 가능성
  * 타임프레임 3/3 일치 + 거래량 증가 → 추세 신뢰도 높음
  * 고점 근접 + 거래량 감소 → 돌파 실패 가능성
  * 4시간봉 MA20 위 + 눌림패턴 + 거래량 감소 → 건강한 조정 후 재진입 가능성
  * RSI 65 이상 + 당일 급등 + 15분봉 음봉 → 단기 과열, 추격 위험
...
[수급 분석] 매집/분배 가능성, 거래량 질 평가 (15분봉 종가가 양봉/음봉 여부 반드시 언급)
[강한 요소] 핵심 3가지
[위험 요소] 핵심 3가지
